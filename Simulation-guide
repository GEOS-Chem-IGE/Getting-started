...
Building executables and running a simulation can be thought of as taking a journey.

Running your program without submitting it as a job is like riding a bicycle on a busy highway: you put yourself and others at risk, and the journey will be slow and unsafe.

Running your program through a job scheduler is like renting a car and choosing the proper lanes: you travel faster, more safely, and you respect the rules of the road so that everyone can move smoothly.

In practical terms, submitting a job means you tell the cluster:

how many nodes (machines) you need,

how many CPUs (or cores) you want to use,

and which script or executable should be run.

This is called a batch job: instead of running your program directly on the cluster, you request resources and the job scheduler runs it for you.

On our system, we use OAR, a job scheduler designed for High-Performance Computing (HPC) clusters. It manages resources, ensures fair usage, and allows your simulation to run efficiently alongside other users’ jobs.
...
```
bettic is a very fast shared filesystem built for clusters that stores data across many servers and disks. Multiple clients can read/write parts of the same file at once
...
oarsub -S ./3_run.sh
```
You can verify your job status with 
´´´
